...
public class Scenes {
	public static Scene scene1() {
		Plain plainShape = new Plain(new Vec(0.0, 0.0, 1.0), new Point(0.0, 0.0, 0.0));
		Material plainMaterial = Material.getMetalMaterial();
		Surface plainSurface = new Surface(plainShape, plainMaterial);
		DirectionalLight dirLight = new DirectionalLight().initDirection(new Vec(0.0, 0.0, -1.0)).initIntensity(new Vec(0.5));
		PointLight pointLight = new PointLight().initIntensity(new Vec(0.9)).initPosition(new Point(2.5, 0.0, 1.0)).initDecayFactors(0.1, 0.05, 1.0);
		Spotlight spotLight = new Spotlight().initIntensity(new Vec(0.9)).initPosition(new Point(-2.5, 0.0, 1.0)).initDecayFactors(0.1, 0.05, 1.0).initDirection(new Vec(-0.1, -0.1, -0.1));
	...
	public static Scene scene2() {
		AxisAlignedBox boxShape1 = new AxisAlignedBox(new Point(0.0, 0.0, 0.0), new Point(1.0, 1.0, 1.0));
		Material boxMat1 = new Material().initKa(new Vec(0.8, 0.05, 0.05)).initKd(new Vec(0.0)).initKs(new Vec(0.9)).initShininess(10).initIsTransparent(false).initRefractionIntensity(0.0);
		Surface boxSurface1 = new Surface(boxShape1, boxMat1);
		DirectionalLight dirLight = new DirectionalLight().initDirection(new Vec(-1.0, -1.0, -1.0)).initIntensity(new Vec(0.9));
		PointLight pointLight1 = new PointLight().initIntensity(new Vec(0.7)).initPosition(new Point(2.0, 2.0, 1.5));
		PointLight pointLight2 = new PointLight().initIntensity(new Vec(0.7)).initPosition(new Point(2.0, 1.0, 0.5));
	...
	public static Scene scene3() {
		AxisAlignedBox boxShape1 = new AxisAlignedBox(new Point(-0.5, 0.0, 0.0), new Point(0.5, 1.0, 1.0));
		Material boxMat1 = new Material().initKa(new Vec(0.01)).initKd(new Vec(0.03)).initKs(new Vec(0.03)).initIsTransparent(true).initRefractionIntensity(1.0);
		Surface boxSurface1 = new Surface(boxShape1, boxMat1);
		AxisAlignedBox boxShape2 = new AxisAlignedBox(new Point(0.0, 0.0, 0.0), new Point(1.0, -1.0, 1.0));
		Material boxMat2 = new Material().initKa(new Vec(0.5, 0.0, 0.0)).initKd(new Vec(0.1)).initKs(new Vec(0.1)).initIsTransparent(true).initRefractionIntensity(0.1);
		Surface boxSurface2 = new Surface(boxShape2, boxMat2);
		AxisAlignedBox boxShape3 = new AxisAlignedBox(new Point(0.0, 0.0, 0.0), new Point(-1.0, -1.0, 1.0));
		Material boxMat3 = new Material().initKa(new Vec(0.0, 0.0, 0.5)).initKd(new Vec(0.1)).initKs(new Vec(0.1)).initIsTransparent(true).initRefractionIntensity(0.1);
		Surface boxSurface3 = new Surface(boxShape3, boxMat3);
		DirectionalLight dirLight = new DirectionalLight().initDirection(new Vec(0.0, -1.0, -1.0)).initIntensity(new Vec(0.7, 0.5, 0.0));
	...
	public static Scene scene4() {
		...
		pyramidScence.initAmbient(new Vec(0.33));
		pyramidScence.initRenderRefarctions(true).initRenderReflections(true).initMaxRecursionLevel(6);
		Point cameraPosition = new Point((double)(2 * boxWidth) * Math.pow(pyramidHeight + 1, 2.0), (double)(2 * boxDepth) * Math.pow(pyramidHeight + 1, 2.0), boxHeight * pyramidHeight);
		Vec towardsVec = new Vec(-1.0, -1.0, 0.0);
		Vec upVec = new Vec(0.0, 0.0, 1.0);
		double distanceFromPlain = 0.25 * cameraPosition.dist(new Point(0.0, 0.0, 0.0));
		pyramidScence.initCamera(cameraPosition, towardsVec, upVec, distanceFromPlain);
		DirectionalLight dirLight = new DirectionalLight().initDirection(new Vec(-0.5, -0.5, -1.0)).initIntensity(new Vec(0.5));
		PointLight pointLight1 = new PointLight().initPosition(new Point(12.0, 4.0, 6.0)).initIntensity(new Vec(1.0));
		PointLight pointLight2 = new PointLight().initPosition(new Point(4.0, 12.0, 6.0)).initIntensity(new Vec(1.0));
		PointLight pointLight3 = new PointLight().initPosition(new Point(-12.0, -20.0, 6.0)).initIntensity(new Vec(1.0));
		pyramidScence.addLightSource(dirLight);
		...
		pyramidScence.addLightSource(pointLight3);
		for (int currentHeight = 0; currentHeight < pyramidHeight; ++currentHeight) {
			int numOfBoxes = (int)Math.pow(2.0, pyramidHeight - 1) - 2 * currentHeight;
			int offsetX = currentHeight * boxWidth;
			int offsetY = currentHeight * boxDepth;
			for (int i = 0; i < numOfBoxes; ++i) {
				for (int j = 0; j < numOfBoxes; ++j) {
					AxisAlignedBox boxShape = new AxisAlignedBox(new Point(offsetX + i * boxWidth, offsetY + j * boxDepth, boxHeight * currentHeight), new Point(offsetX + (i + 1) * boxWidth, offsetY + (j + 1) * boxDepth, (currentHeight + 1) * boxHeight));
					Material boxMat = Material.getRandomMaterial();
					Surface boxSurface = new Surface(boxShape, boxMat);
					pyramidScence.addSurface(boxSurface);
				}
	...
	public static Scene scene5() {
		Sphere sphereShape1 = new Sphere(new Point(0.5, 0.5, 0.5), 0.5);
		Material sphereMat1 = Material.getRandomMaterial();
		Surface sphereSurface1 = new Surface(sphereShape1, sphereMat1);
		Sphere sphereShape2 = new Sphere(new Point(-0.5, 0.5, 0.5), 0.5);
		Material sphereMat2 = Material.getRandomMaterial();
		Surface sphereSurface2 = new Surface(sphereShape2, sphereMat2);
		Sphere sphereShape3 = new Sphere(new Point(0.5, -0.5, 0.5), 0.5);
		Material sphereMat3 = Material.getRandomMaterial();
		Surface sphereSurface3 = new Surface(sphereShape3, sphereMat3);
		Sphere sphereShape4 = new Sphere(new Point(-0.5, -0.5, 0.5), 0.5);
		Material sphereMat4 = Material.getRandomMaterial();
		Surface sphereSurface4 = new Surface(sphereShape4, sphereMat4);
		DirectionalLight dirLight = new DirectionalLight().initDirection(new Vec(0.0, 0.0, -1.0)).initIntensity(new Vec(0.7));
