...
public class Scenes {
	...
	public static Scene scene7() {
		DirectionalLight dirLight = new DirectionalLight().initDirection(new Vec(0.0, 0.0, -1.0)).initIntensity(new Vec(0.7));
		PointLight pointLight = new PointLight().initIntensity(new Vec(0.7))
				.initPosition(new Point(1.0, 0.0, 1.0)).initDecayFactors(0.1,0.05,1.0);
		Sphere sphereShape1 = new Sphere(new Point(0.5, 0.5, 0.5), 0.5);
		Material sphereMat1 = Material.getRandomMaterial();
		Surface sphereSurface1 = new Surface(sphereShape1, sphereMat1);

		return new Scene().initAmbient(new Vec(1.0))
				.initCamera(new Point(0.0, 0.0, 2.0), new Vec (0.0,0.0,-1.0), new Vec (1.0, 1.0, 0.0), 1.0).addLightSource(dirLight).addLightSource(pointLight).addSurface(sphereSurface1).initName("scene7").initAntiAliasingFactor(1).initRenderRefarctions(true).initRenderReflections(true).initMaxRecursionLevel(6);
	}
}
